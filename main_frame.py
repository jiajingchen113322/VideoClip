import os
import sys
sys.path.append('/home/jchen152/workspace/Deep_Insigth_Project/Zero_Shot/CLIP-main')
import clip
import numpy as np
import torch
...
from others.classifier import classifier as classifier_net

# ============== Get Configuration =================
def get_arg():
    cfg=argparse.ArgumentParser()
    cfg.add_argument('--exp_name',default='training_lr_3*1e-4_frame_fold0_newcls')
    ...
    cfg.add_argument('--batch_size',default=50)
    cfg.add_argument('--fold',default=0)
    cfg.add_argument('--video_based',default=False)
...
text_list=clip.tokenize(text_list).to(cfg.device)
...
def main():
    ...
    # model.load_state_dict(torch.load('/data1/jiajing/worksapce/Project/Zero_Shot/S3D_HowTo100M/s3d_howto100m.pth'))
    ...
    if cfg.train:
        train_model(model,classifier,train_loader,valid_loader,cfg.exp_name,cuda)
...
def train_model(model,classifier,train_loader,valid_loader,exp_name,cuda_n):
    ...
    def train_one_epoch():
        ...
        #真正训练这里应该解封
        epsum=run_one_epoch(model,classifier,iterations,"train",loss_func=loss_func,optimizer=optimizer,loss_interval=10)
    ...
    def eval_one_epoch():
        ...
        epsum=run_one_epoch(model,classifier,iteration,"valid",loss_func=loss_func)
...
def run_one_epoch(model,classifier,tqdm_iter,mode,loss_func=None,optimizer=None,loss_interval=10):
    if mode=='train':
        model.eval()
        classifier.train()
    else:
    ...
    for i,(x_cpu,y_cpu) in enumerate(tqdm_iter):
        ...
        with torch.no_grad():
            ...
            img_feat=img_feat.type(torch.float32)
            text_features=text_features.type(torch.float32)

        img_feat_final,text_features_final=classifier(img_feat,text_features)

        # video_output=model(x)
        # text_output=model.text_module(text_list)
        # video_embedding = video_output['video_embedding']
        # text_embedding = text_output['text_embedding']
        logits = torch.matmul(text_features_final, img_feat_final.t()).t()
